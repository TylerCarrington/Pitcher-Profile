...
- [x] **1.1. Add Router Library / Foundation**:
...
- [x] **1.2. Configure Server & Vite Fallback**:
...
- [x] **2.1. URL-Driven Team Hydration in `TeamContext`**:
...
- [x] **2.2. URL-Driven Event Hydration in `EventContext`**:
...
- [x] **2.3. Asynchronous / Cold-Start Loading States**:
...
- [x] **2.4. Invalid Entity Handling & Graceful Fallbacks**:
...
- [x] **3.1. Team Switcher & Tab Navigation**:
...
- [x] **3.2. Event Lifecycle Transitions**:
...
- [x] **3.3. Browser Back / Forward Button Handling**:
...
- [x] **4.1. Invite Link Handler (`/join/:inviteCode`)**:
...
- [x] **4.2. Post-Auth Return Path Preservation**:
...
- [x] **4.3. Admin Route Protection (`/admin`)**:
...
- [x] **5.1. Refresh Verification Matrix**:
  - [x] **Test Case 1 (Live Game Refresh):** Log 15 pitches for Pitcher A in `/events/:eventId`. Press browser Refresh (F5). Ensure app reloads at `/events/:eventId` with Pitcher A selected, 15 pitches on the scoreboard, and pitch history intact.
  - [x] **Test Case 2 (Event Review Refresh):** Open `/events/:eventId/review`. Press Refresh. Ensure scouting summary, strike zone heatmap, and pitcher breakdown load immediately.
  - [x] **Test Case 3 (Roster vs Events Tab Refresh):** Select "Events & Games" tab (`/teams/:teamId/events`). Press Refresh. Ensure user remains on the Events tab rather than defaulting back to Roster.
  - [x] **Test Case 4 (Offline Refresh):** Go offline (airplane mode). Refresh on `/events/:eventId`. Verify the app loads from service worker / local storage cache with zero errors.
  - [x] **Test Case 5 (Direct Invite Link):** Paste `https://domain.app/join/ABC123XYZ` in a new tab. Verify team is joined and user is taken to team roster.
  - [x] **Test Case 6 (Back/Forward Navigation):** Navigate Team Hub -> Live Event -> Review -> Team Hub. Step backward and forward through browser history; verify correct views mount without errors.
