...
export const CoachSwitcher: React.FC<CoachSwitcherProps> = (props) => {
  const auth = useAuth();
  const currentCoach = props.currentCoach ?? auth.currentCoach;
  const allCoaches = props.allCoaches ?? auth.allCoaches;
  const onSelectCoach = props.onSelectCoach ?? auth.switchCoach;
  const onSignOut = props.onSignOut ?? auth.signOut;
  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 text-xs font-semibold transition cursor-pointer"
      >
      ...
      {isOpen && (
        ...
        >
            ...
            {allCoaches.map((c) => {
              ...
              return (
                <button
                  ...
                  className={`w-full px-3 py-2 text-left text-xs flex items-center justify-between gap-2 hover:bg-slate-50 transition cursor-pointer ${
          ...
          {onSignOut && (
            <div className="pt-1.5 mt-1.5 border-t border-slate-100 px-1">
              <button
                type="button"
                id="sign-out-btn"
                onClick={() => {
                ...
                className="w-full px-2.5 py-1.5 text-xs text-rose-600 hover:bg-rose-50 rounded-lg font-semibold flex items-center gap-2 transition cursor-pointer"
              >
